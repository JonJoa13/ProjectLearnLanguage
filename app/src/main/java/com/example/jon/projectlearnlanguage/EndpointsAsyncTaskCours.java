...
public class EndpointsAsyncTaskCours extends AsyncTask<Void, Void, List<Cours>> {
    private static CoursApi coursApi = null;
    private static final String TAG = EndpointsAsyncTaskCours.class.getName();
    private Cours cours;

    EndpointsAsyncTaskCours(){}

    public EndpointsAsyncTaskCours(Cours cours){
        this.cours = cours;
    }
    ...
    protected List<Cours> doInBackground(Void... params) {
        ...
        try{
            ...
            // For instance insert
            if(cours != null){
                coursApi.insert(cours).execute();
                Log.i(TAG, "insert cours");
            }
