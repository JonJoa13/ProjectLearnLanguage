...
public class EndpointsAsyncTaskChoiceUpdate extends AsyncTask<Void, Void, List<Choice>> {
    private static ChoiceApi choiceApi = null;
    private static final String TAG = EndpointsAsyncTaskChoice.class.getName();
    private Choice choice;

    EndpointsAsyncTaskChoiceUpdate(){}

    public EndpointsAsyncTaskChoiceUpdate(Choice choice){
        this.choice = choice;
    }
    ...
    protected List<Choice> doInBackground(Void... params) {
        ...
        try{
            ...
            // For instance insert
            if(choice != null){
                choiceApi.update(choice.getId(),choice).execute();
                Log.i(TAG, "update choice");
            }
