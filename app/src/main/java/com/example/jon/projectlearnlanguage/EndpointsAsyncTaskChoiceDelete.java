...
public class EndpointsAsyncTaskChoiceDelete extends AsyncTask<Void, Void, List<Choice>> {
    private static ChoiceApi choiceApi = null;
    private static final String TAG = EndpointsAsyncTaskChoice.class.getName();
    private Choice choice;

    EndpointsAsyncTaskChoiceDelete(){}

    public EndpointsAsyncTaskChoiceDelete(Choice choice){
        this.choice = choice;
    }
    ...
    protected List<Choice> doInBackground(Void... params) {
        ...
        try{
            ...
            // For instance insert
            if(choice != null){
                choiceApi.remove(choice.getId()).execute();
                Log.i(TAG, "delete choice");
            }
