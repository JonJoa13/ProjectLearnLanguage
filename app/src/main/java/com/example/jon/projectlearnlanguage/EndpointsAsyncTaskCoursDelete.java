...
public class EndpointsAsyncTaskCoursDelete extends AsyncTask<Void, Void, List<Cours>> {
    private static CoursApi coursApi = null;
    private static final String TAG = EndpointsAsyncTaskCours.class.getName();
    private Cours cours;

    EndpointsAsyncTaskCoursDelete(){}

    public EndpointsAsyncTaskCoursDelete(Cours cours){
        this.cours = cours;
    }
    ...
    protected List<Cours> doInBackground(Void... params) {
        ...
        try{
            ...
            // For instance insert
            if(cours != null){
                coursApi.remove(cours.getId()).execute();
                Log.i(TAG, "delete cours");
            }
